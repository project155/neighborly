...
class HygieneissuesReportPage extends StatefulWidget {
  @override
  _HygieneissuesReportPageState createState() => _HygieneissuesReportPageState();
}

class _HygieneissuesReportPageState extends State<HygieneissuesReportPage>
  ...
  Future<void> _fetchReports() async {
    try {
      // Obtain user's current location for radius filtering.
      ...
      var snapshot = await _firestore
          .collection('reports')
          .where('category', isEqualTo: 'Hygiene Issues')
          ...
          child: SlideTransition(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(
                color: Color.fromARGB(255, 255, 78, 19),
        ...
        child: Row(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Icon(Icons.cleaning_services, size: 30, color: Colors.redAccent),
                SizedBox(width: 8),
                Text("Hygiene Issues Reports", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.black87)),
              ],
            ),
            IconButton(
              icon: Icon(Icons.search, color: Colors.black87),
              onPressed: () {
                showSearch(context: context, delegate: HygieneissuesReportSearchDelegate(reports: _reports));
              },
            ),
          ],
        ),
  ...
  Widget _buildTappableImage(String imageUrl) {
    return GestureDetector(
      onTap: () {
        Navigator.push(context, MaterialPageRoute(builder: (_) => FullScreenImagePage(imageUrl: imageUrl)));
      },
                  ...
                  child: ListView(
                    children: comments.map((comment) {
                      if (comment is Map && comment.containsKey('name')) {
                        return ListTile(
                          leading: CircleAvatar(backgroundImage: AssetImage('assets/images/anonymous_avatar.png')),
                ...
                : _reports.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Wrap the report card in an InkWell for map redirection.
                          return InkWell(
                            onTap: () {
                              if (report['location'] != null) {
                                double lat = (report['location']['latitude'] ?? 0).toDouble();
                                      ...
                                      children: [
                                        ...
                                        Row(
                                          ...
                                          children: [
                                            ...
                                            Text("Urgency: $urgency", style: TextStyle(color: Colors.redAccent)),
                                          ],
                                        ),
          ...
          items: widget.imageUrls.map((image) {
            return ClipRRect(
              ...
              child: GestureDetector(
                onTap: () {
                  Navigator.push(context, MaterialPageRoute(builder: (_) => FullScreenImagePage(imageUrl: image)));
                },
                child: Image.network(
                  image,
                  fit: BoxFit.cover,
...
class HygieneissuesReportSearchDelegate extends SearchDelegate {
  final List<Map<String, dynamic>> reports;
  HygieneissuesReportSearchDelegate({required this.reports});
  ...
  Widget buildResults(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          leading: Icon(Icons.cleaning_services, color: Colors.redAccent),
          title: Text(report['title'] ?? "No Title"),
          subtitle: Text(report['description'] ?? "No Description"),
          onTap: () {
            close(context, null);
            Navigator.push(context, MaterialPageRoute(builder: (context) => HygieneissuesReportDetailPage(report: report)));
          },
        );
      },
    );
...
class HygieneissuesReportDetailPage extends StatelessWidget {
  final Map<String, dynamic> report;
  HygieneissuesReportDetailPage({required this.report});
  @override
  Widget build(BuildContext context) {
    List<String> imageUrls = [];
    if (report['imageUrl'] is List) {
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 5),
                  Text("Urgency: ${report['urgency'] ?? "Normal"}", style: TextStyle(fontSize: 16, color: Colors.redAccent)),
                ],
              ),
            )
          ],
