...
class NarcoticsReportClonePage extends StatefulWidget {
  @override
  _NarcoticsReportClonePageState createState() => _NarcoticsReportClonePageState();
}

class _NarcoticsReportClonePageState extends State<NarcoticsReportClonePage>
          ...
          children: [
            ...
            Row(
              ...
              children: [
                ...
                SizedBox(width: 8),
                Text("Narcotics Reports", style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Colors.black87)),
              ],
            ),
            IconButton(
              icon: Icon(Icons.search, color: Colors.black87),
              onPressed: () { showSearch(context: context, delegate: NarcoticsReportSearchDelegate(reports: _reports)); },
            ),
          ],
        ),
      ),
                              ...
                              children: [
                                ...
                                Row(
                                  ...
                                  children: [
                                    ...
                                    Text("Urgency: $urgency", style: TextStyle(color: Colors.cyan)),
                                  ],
                                ),
...
class UploaderDetailsPage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text("Uploader Details"), backgroundColor: Colors.cyan),
        ...
        Row(
          ...
          children: widget.imageUrls.asMap().entries.map((entry) {
            return Container(
              ...
              decoration: BoxDecoration(shape: BoxShape.circle, color: _current == entry.key ? Colors.cyan : Colors.grey.shade400),
            );
          }).toList(),
        ),
      ],
...
class NarcoticsReportSearchDelegate extends SearchDelegate {
  final List<Map<String, dynamic>> reports;
  NarcoticsReportSearchDelegate({required this.reports});
  ...
  Widget buildResults(BuildContext context) {
    ...
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        return ListTile(
          leading: Icon(Icons.local_pharmacy, color: Colors.cyan),
          title: Text(report['title'] ?? "No Title"),
          subtitle: Text(report['description'] ?? "No Description"),
          onTap: () {
            close(context, null);
            Navigator.push(context, MaterialPageRoute(builder: (context) => NarcoticsReportDetailPage(report: report)));
          },
        );
      },
    );
...
class NarcoticsReportDetailPage extends StatelessWidget {
  final Map<String, dynamic> report;
  NarcoticsReportDetailPage({required this.report});
  @override
  Widget build(BuildContext context) {
    List<String> imageUrls = [];
    if (report['imageUrl'] is List) {
    ...
    return Scaffold(
      appBar: AppBar(backgroundColor: Colors.cyan, title: Text(report['title'] ?? "Report Details")),
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          ...
          Padding(
            ...
            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              SizedBox(height: 5),
              Text("Urgency: ${report['urgency'] ?? "Normal"}", style: TextStyle(fontSize: 16, color: Colors.cyan)),
            ]),
          )
        ]),
      ),
