...
class FoodIssuesReportClonePage extends StatefulWidget {
  @override
  _FoodIssuesReportClonePageState createState() => _FoodIssuesReportClonePageState();
}

class _FoodIssuesReportClonePageState extends State<FoodIssuesReportClonePage>
  ...
  Future<void> _fetchReports() async {
    try {
      ...
      // Fetch all 'food issues' reports.
      var snapshot = await _firestore
          .collection('reports')
          .where('category', isEqualTo: 'Food Issues')
          ...
          children: [
            ...
            IconButton(
              ...
              onPressed: () {
                showSearch(
                  context: context,
                  delegate: FoodIssuesReportSearchDelegate(reports: _reports),
                );
              },
            ),
          ],
          ...
          children: widget.imageUrls.asMap().entries.map((entry) {
            return Container(
              ...
              decoration: BoxDecoration(
                ...
                color: _current == entry.key ? Colors.green : Colors.grey.shade400,
              ),
            );
          }).toList(),
        ),
...
// Custom SearchDelegate for food issues reports.
class FoodIssuesReportSearchDelegate extends SearchDelegate {
  final List<Map<String, dynamic>> reports;

  FoodIssuesReportSearchDelegate({required this.reports});
        ...
        return ListTile(
          ...
          onTap: () {
            ...
            Navigator.push(
              context,
              MaterialPageRoute(
                builder: (context) => FoodIssuesReportDetailPage(report: report),
              ),
            );
          },
        );
...
// Detail page for a food issues report.
class FoodIssuesReportDetailPage extends StatelessWidget {
  final Map<String, dynamic> report;

  FoodIssuesReportDetailPage({required this.report});
