                ...
                ListTile(
                  ...
                  onTap: () {
                    Navigator.push(
                      context,
                      MaterialPageRoute(
                        builder: (context) => AuthorityReportPage(),
                      ),
                    );
                  },
                ),
...
class AuthorityReportPage extends StatefulWidget {
  @override
  _AuthorityReportPageState createState() => _AuthorityReportPageState();
}

class _AuthorityReportPageState extends State<AuthorityReportPage>
  ...
  // Fetch reports and then use the uploaderId to get name and email from the users collection.
  Future<void> _fetchReports() async {
    try {
      ...
      for (var doc in snapshot.docs) {
        ...
        data['id'] = doc.id;
        print("Report ${doc.id} data: $data");

        // Fetch user details from the users collection using uploaderId
        if (data.containsKey('uploaderId')) {
          String uploaderId = data['uploaderId'];
          print("Fetching user for uploaderId: $uploaderId");
          DocumentSnapshot userSnapshot =
              await _firestore.collection('users').doc(uploaderId).get();
          if (userSnapshot.exists && userSnapshot.data() != null) {
            Map<String, dynamic> userData =
                userSnapshot.data() as Map<String, dynamic>;
            print("User Data for $uploaderId: $userData");
            data['name'] = userData['name'] ?? "Unknown";
            data['email'] = userData['email'] ?? "Unknown";
          } else {
            print("No user data found for uploaderId: $uploaderId");
            data['name'] = "Unknown";
            data['email'] = "Unknown";
          }
        } else {
      ...
      // Add markers to the map for each report location.
      _markers.clear();
        ...
        child: Row(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Icon(Icons.report, size: 30, color: Colors.blueAccent),
                SizedBox(width: 8),
                Text(
                  "Reports",
                  style: TextStyle(
            ...
            IconButton(
              ...
              onPressed: () {
                showSearch(
                  context: context,
                  delegate:
                      AuthorityReportSearchDelegate(reports: _reports),
                );
              },
            ),
          ],
                ...
                : _reports.isEmpty
                    ...
                    : ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          // Use the fetched "name" and "email" from the users collection.
                          String name = report['name'] ?? "Unknown";
                          String email = report['email'] ?? "Unknown";
...
class AuthorityReportSearchDelegate extends SearchDelegate {
  final List<Map<String, dynamic>> reports;

  AuthorityReportSearchDelegate({required this.reports});
        ...
        return ListTile(
          ...
          onTap: () {
            ...
            Navigator.push(
              ...
              MaterialPageRoute(
                builder: (context) =>
                    AuthorityReportDetailPage(report: report),
              ),
            );
          },
        );
...
class AuthorityReportDetailPage extends StatelessWidget {
  final Map<String, dynamic> report;

  AuthorityReportDetailPage({required this.report});
            ...
            Padding(
              ...
              child: Column(
                ...
                children: [
                  ...
                  SizedBox(height: 10),
                  // Display fetched user details (name and email).
                  Text(
                    "Uploaded by: ${report['name'] ?? "Unknown"}",
                    style: TextStyle(fontSize: 16),
                  ),
                  Text(
                    "Email: ${report['email'] ?? "Unknown"}",
                    style: TextStyle(fontSize: 16),
                  ),
                ],
              ),
